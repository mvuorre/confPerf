...
authornote: |
  This research was supported, in part, by Institute of Education Science grant (R305A150467) awarded to JM. The authors are solely responsible for the content of this article. The data and code supporting this manuscript are available at https://osf.io/np3cu/.
abstract: |
  Are unskilled people less aware of their (lack of) skill than are their more skilled peers? The idea that unskilled people are also unaware (the "Dunning-Kruger effect") is supported by data from many domains, and has largely concerned people's predictions or retrodictions of their performance in a given task. However, we suggest that if "awareness" is operationalized differently, as relative metacognitive accuracy, the apparent correlation between skill and awareness may be artificially induced by common methods of multiple-choice testing. In the metacognitive research tradition, relative accuracy---the trial-by-trial association between response accuracy and metacognitive endorsement of that response's accuracy---is a more specific measure of awareness than is a one-shot retrodiction or prediction of performance; it directly answers the question "how well do you know that you (don't) know". 
  We found a robust positive correlation between resolution and performance in six multiple-choice mathematics experiments, evidencing the "unskilled and unaware" phenomenon. However, when we directly compared multiple-choice to free-response testing in an otherwise identical task, we found that the correlation between skill and awareness disappeared in free-response testing. We then examined the hypothesis that guessing in a multiple choice test would directly lead to an illusory "unskilled and unaware" effect using monte carlo simulation. The results of that simulation strongly suggested that the correlation between skill and awareness observed in multiple choice testing is an artifact due to guessing, and not indicative of decreased awareness in less skilled individuals. We also discuss the implications of these findings to measurement of metacognitive accuracy.
  
keywords: "metacognition, confidence, measurement, unskilled and unaware"
...
To successfully improve learning, it is important that people are able to monitor what they know and what they don't know [@BjorkMemoryMetamemoryConsiderations1994; @FlavellMetacognitioncognitivemonitoring1979; @MetcalfeMetacognitiveJudgmentsControl2009; @NelsonMetamemoryTheoreticalFramework1990; @NelsonUtilizationMetacognitiveJudgments1994]. This metacognitive monitoring and knowledge, when accurate, may be used to select appropriate items for further study, seek assistance to improve comprehension, or otherwise change behavior when external feedback is not available. To paraphrase the oft-quoted adage: If you don't know what you don't know, you can't fix it. It is therefore of paramount importance that people be metacognitively accurate.

However, it has repeatedly been found, across a wide range of tasks, that people who are less skilled at a given task are also less aware of their performance in the task (the "unskilled and unaware effect", also known as the "Dunning-Kruger effect"; @KrugerUnskilledUnawareIt1999). For example, @KrugerUnskilledUnawareIt1999 (Study 3) had volunteers read sentences that missed parts. For each sentence, their task was to determine whether the sentence was grammatical or not, and if not, which of four different rewordings would make it grammatical. Afterwards, the participants estimated how many items they completed succesfully. The findings indicated a dramatic unskilled and unaware effect: The worst-performing participants grossly overestimated their test scores, whereas better performing participants were more accurate in their self-evaluations. It appears, then, that unskilled individuals may suffer from a metacognitive mismatch by which they tend to dramatically overestimate their skill, which potentially negative implications to learning, adjustment, and behavior.

This popular measure of self-evaluation by which participants predict or retrodict their performance in a task is typically called metacognitive calibration, absolute accuracy, bias, or over/underconfidence [@DunloskyMetacognition2009; @Lichtensteinthosewhoknow1977]. However, in the metacognitive research tradition, another, related but not identical, measure is typically used to measure metacognitive awareness. 

The relative accuracy, or *resolution*, of metacognitive judgments measures the extent to which people can discriminate whether they answered (or will answer) a particular question accurately, on an item-by-item basis. Arguable, it is a more suitable operationalization of "awareness" than is calibration, because it indicates an online, in-the-moment, evaluation of one's performance in a task. A strong positive relation between a participant's ratings about how sure they are that they are, or will be, correct on particular items and their actual correctness on those items indicates good resolution. Broadly, research has focused on two types of metacognitive ratings: Prospective ratings, such as judgments of learning (JOL), feelings of knowing (FOK) or ease of learning judgments (EOL), indicate how likely the participant thinks that a later retrieval or answer will be correct. Retrospective ratings indicate the participant's judgment of the accuracy of a just-provided answer, typically in the form of judgments of confidence. Although these judgments are distinct, for clarity we refer to all as judgments of confidence (in the accuracy of one's answer). Typically, the correspondence between confidence and accuracy is referred to as metacognitive resolution, relative accuracy [@DunloskyMetacognition2009], or sensitivity [@FlemingHowmeasuremetacognition2014].
...
Importantly, the unskilled and unaware phenomenon is typically studied using calibration as a measure of awareness.  However, that metric is known to suffer from various drawbacks that can lead to artificial correlations between skill and awareness [@FeldEstimatingrelationshipskill2017; @KrajcAreunskilledreally2008; @McIntoshWiseClarifyingrole2019]. A continuing debate has concerned the extent to which observed unskilled and unaware effects are due to properties of the measurement or reflect actual relations between skill and awareness. It is recognized that both play a role in the observed unskilled and unaware effects.

However, metacognitive resolution is typically measured with metrics that are asserted to be independent of the participants' performance, skill, or knowledge in the primary task. In an influential article @Nelsoncomparisoncurrentmeasures1984 recommended the Goodman-Kruskal gamma correlation [@GoodmanMeasuresAssociationCross1954] as a measure of resolution, partly because it is theoretically independent of task performance [see also @GonzalezMeasuringordinalassociation1996]. Other measures of resolution include Pearson's *r* [@NelsonNorms300generalinformation1980], the phi correlation [@KornellTransferMetacognitiveSkills2007], and signal detection theoretic measures [@benjamin2008measurement; @HighamNewimprovedgamma2018]. Although computationally different, these metrics measure the trial-by-trial association between response accuracy and metacognitive rating in a way that is usually assumed to quantify metacognitive ability independent of task performance. However, it remains a possibility that metacognitive resolution is empirically related to task performance in a way that would show the unskilled and unaware effect over and beyond its effects on calibration. Such a finding would have dramatic consequences to many areas where metacognitive skills are important, including self-directed studying, to which we turn next.
...
```{r math-example, fig.cap = "Example mathematics problem from Experiment 1."}
...
apa_table(
  tab,
  "Measures of mathematics performance and resolution in Experiment 1. Numbers indicate means with 95\\% CIs in square brackets."
)
```
...
```{r sim-rfig, fig.cap = "Results of the simulation study at various levels of resolution (color), knowledge, and guessing, for da and gamma."}
sims_long %>% 
...
We then asked whether resolution in Experiments 1A-F was different from perfect if guessing was accounted for. We tested participants' *d*~a~s against the estimates from the simulation with Guessing = 0.25 and Resolution as the greatest value used in our simulations (2.5), at the level of the participants' performance, with a paired *t*-test. The simulated mean estimates of *d*~a~ from near-perfect resolution and Guessing = 0.25 were greater than the participants' observed *d*~a~s (`r res`): Participants did not have perfect metacognitive resolution. Further, better performing participants were further below near-perfect resolution than were poorer performing participants. Using gamma as the resolution metric yielded identical results.

In summary, our simulation study suggested that in studies of metacognition using multiple-choice testing, metrics of resolution will necessarily correlate with primary task performance. However, we assumed a generative model of confidence that relied on random guessing in the primary task. One concern with this model is that such random guessing, while intuitively appealing, may not be empirically plausible [e.g. @HighamNoSpecialsignal2007]. In fact, one study using SAT (five alternative multiple-choice) materials found that a signal detection theoretic (SDT) model described performance better than did a high-threshold model (i.e. model with random guessing; @HighamNoSpecialsignal2007). Specifically, high-threshold theory (as implied in our simulations) predicts that accuracy at the lowest level of confidence should not drop below 1/K (K = number of response alternatives). In contrast, as found in that study and predicted by SDT, accuracy was below 20% for low confidence responses, thus discrediting the random guessing assumption [@HighamNoSpecialsignal2007]. 

```{r accuracy-at-confidence, eval = TRUE}
tmp <- bind_rows(
  math %>% 
    filter(Experiment %in% c("1A", "1B", "1C")) %>% 
    group_by(Experiment, subject) %>% 
    mutate(con = bin_ratings(con, 1, 6, 6)) %>% 
    group_by(Experiment, subject, con) %>% 
    summarise(p = mean(accuracy)),
  math %>% 
    filter(Experiment %in% c("1D", "1F")) %>% 
    group_by(Experiment, subject) %>% 
    mutate(con = bin_ratings(con, 0, 1, 6)) %>% 
    group_by(Experiment, subject, con) %>% 
    summarise(p = mean(accuracy)),
  math %>% 
    filter(Experiment %in% c("1E")) %>% 
    group_by(Experiment, subject) %>% 
    mutate(con = bin_ratings(con, 0, 10, 6)) %>% 
    group_by(Experiment, subject, con) %>% 
    summarise(p = mean(accuracy))
) %>% 
  left_join(select(math_metrics, Experiment, subject, proportion_correct)) %>% 
  ungroup() %>% 
  mutate(Performance = cut2(proportion_correct, g = 3))
res <- tmp %>% 
  filter(con==1) %>% 
  group_by(Performance) %>% 
  group_modify(~tidy(t.test(.$p, mu = 0.25))) %>% 
  glue_data("{round(estimate*100, 1)}%, 95%CI = [{round(conf.low*100, 1)}, {round(conf.high*100, 1)}]")

# tmp %>%   
#   ggplot(aes(con, p, col = Performance)) +
#   scale_y_continuous(
#     "Proportion correct",
#     limits = 0:1
#   ) +
#   labs(x = "Confidence") +
#   stat_summary(geom = "line", aes(group=Performance)) +
#   stat_summary()
```

To begin to address this issue, we took the data of Experiments 1A-F, and asked if performance did indeed drop below 25% at the lowest level of confidence. We did this analysis separately for performance tertiles of participants, because good performing students should necessarily guess less than poor performers. In support of the guessing hypothesis, none of the performance tertiles had performance below 25% at the lowest level of confidence (Low performers: `r res[1]`, medium performers: `r res[2]`, high performers: `r res[3]`). In fact, as can be seen, the lowest performers were almost exactly at 25% at the lowest level of confidence. Nevertheless, these analyses were preliminary, and a stronger test of our hypothesis would be to experimentally contrast tasks where guessing was possible and where it was not. Therefore, we put the idea that resolution metrics are related to domain knowledge in multiple-choice tasks, possibly due to guessing, to an experimental test. In Experiment 2, we directly compared indices of metacognitive resolution, and their relation to domain skill, between free-response trials and multiple-choice trials. 
...
We recruited 100 participants through the Prolific (https://prolific.co) online platform. `r tsex` participants were female, and age ranged from `r tage[1]` to `r tage[2]` years. We recruited participants currently residing in the US, and `r tfl` reported English as their first language. All participants provided informed consent, and were paid $3.64. We determined the sample size with a power simulation based on the results of the simulation study described above, such that we would achieve 90% power for detecting a difference in the resolution-performance correlations between 2AFC and free response conditions.

### Materials

We collected general knowledge questions from @TauberGeneralknowledgenorms2013 and from previous experiments conducted in our laboratory. Based on those previous studies, we selected 120 questions of varying degrees of difficulty, and found the most frequently endorsed commission errors to be used as the false response options in the 2AFC condition. We also screened the questions (and response alternatives) for potentially offensive or outdated material, and replaced unsuitable false response options with new ones. As an example, one question was "What college or university, founded in 1636, is the oldest in the united states?". (Correct response: "Harvard", incorrect alternative: "Columbia").
...
tmp2 <- trivia_metrics %>%
  gather(
    key, 
    value, 
    proportion_correct, mean_rating, zroc_auc_gamma, zroc_da,
  ) %>% 
...
bind_rows(tmp2, tmp1) %>% 
  apa_table(
    caption = "Measures of general information test performance and metacognition in Experiment 2. Numbers indicate means with 95\\% CIs in square brackets."
  )
```

We then turned to the second part of our hypothesis; that resolution would be spuriously correlated to performance in a 2AFC task. We correlated the three resolution metrics to performance in each conditions, using the nonparametric tau correlation. The correlation was significant for all three resolution metrics in the 2AFC condition (Table \@ref(tab:trivia-ctab)). However, it was much smaller in the free response condition, where it was only significant for the concordant-discordant pairs based gamma (Figure \@ref(fig:trivia-rfig)). 

```{r trivia-rfig, fig.cap = "Scatterplot of participants' resolution metrics and proportions correct across the two conditions in Experiment 2. Lines of best fit and 95\\% CIs are from an ordinary linear regression model."}
trivia_metrics %>%
  gather(key, value, zroc_auc_gamma, zroc_da, gamma_old) %>%
  left_join(new_names) %>% 
  ggplot(aes(proportion_correct, value)) +
  ...
  geom_point(shape = 21, alpha = .5, size = 1) +
  geom_smooth(method = "lm", col = "black", size = .7) +
...
In six multiple-choice mathematics testing experiments, we found a positive relation between mathematics performance and metacognitive resolution. Following @schwartz1994methodological, we suspected that an underlying reason for the correlation may be that some correct responses were lucky guesses. We conducted simulations based on the assumption that the response alternative to unknown items is randomly guessed. This assumption leads to a strong correlation between resolution and domain skill, even when the two abilities are truly uncorrelated. However, the random guessing hypothesis implies a high threshold model of performance on the primary task, which has been shown to be a poorer fit to data than is signal detection theory [@HighamNoSpecialsignal2007]. However, signal detection theoretic analyses of multiple-choice tasks with more than two alternatives are rare, and researchers commonly use proportion correct as a measure of performance instead [see @DeCarlosignaldetectionapproach2012]. Therefore, we then experimentally verified that i) resolution metrics were smaller in 2AFC than in free-response testing, and that ii) there was a strong positive correlation between resolution metrics and performance in 2AFC, but not in free-response testing, as predicted by the guessing assumption.

Our results bear relevance to the finding that people oftentimes are "unskilled and unaware of it" (also known as the Dunning-Kruger effect, @KrugerUnskilledUnawareIt1999): People with poor performance are less aware of their lack of skill, compared to better performing individuals. The "unskilled and unaware" effect is typically examined in people's global self-assessments (e.g. predictions or retrodictions of overall performance in a task). As such, the Dunning-Kruger effect pertains to the accuracy of metacognitive calibration, a different measure of metacognitive accuracy than discussed in the present work [@DunloskyMetacognition2009; @Lichtensteinthosewhoknow1977] (i.e. poor-performing individuals are overconfident). Our results suggest that guessing, if not appropriately taken into account, might actually work against the Dunning-Kruger effect. As an extreme example, consider a participant in a 2AFC task who performs at chance and subsequently estimates her skill in the task at 0%. Given the random guessing assumption, her observed percentage correct is 50%, leading to an observed underconfidence of 50%. A participant who truly knows the answer to all items, and knows that she knows (gives an average confidence rating of 100%), will not be impacted by correct guesses and thus will not show an underconfidence effect. This issue, however, is easily corrected for by using 1/K (e.g. 25% in 4AFC) as the lowest limit of the confidence scale. Related to this line of reasoning, various statistical and scale-effect explanations for the Dunning-Kruger effect have been explored in the literature [@FeldEstimatingrelationshipskill2017; @KrajcAreunskilledreally2008; @McIntoshWiseClarifyingrole2019].

More importantly, the "unskilled and unaware" idea applies equally well to the within-subject confidence-accuracy relationship (metacognitive resolution), as investigated in the current work. In those terms, "unskilled and unaware" implies that individuals with poorer performance would be metacognitively less sensitive to the accuracy of their own answers, on a trial-by-trial basis. When "awareness" in "unskilled and unaware" is conceptualized as metacognitive resolution, our results strongly imply that an "unskilled but unaware" effect will appear simply because performance and resolution metrics are inherently related in multiple-choice tasks. 

More generally, the important implication of our results is that whenever a process-pure measurement of metacognitive resolution is desired---that is, one that is not inherently related to task performance---researchers would benefit from avoiding tasks that allow correct guessing (e.g. multiple-choice tasks). Ignoring this idea runs the risk of, for example, observing differences in metacognition between conditions, individuals, or groups because of differences in task performance rather than metacognition per se.

Related to the current work, @Maniscalcosignaldetectiontheoretic2012 suggested a measure of relative metacognitive accuracy that explicitly models the relation between primary task performance and metacognition. Based on an analysis showing that sensitivity and response bias in the primary task impact measures of metacognitive performance when studied in a signal detection task [@GalvinTypetaskstheory2003], this new measure has the attractive quality that it is measured in the same units as the primary task performance (i.e. *d'*). This metric, meta-*d'*, allows normalizing metacognitive performance with primary task perfomance, leading to a measure of metacognitive "efficiency" ($\frac{\text{meta-}d^\prime}{d^\prime}$), which is not inherently constrained by primary task performance or bias. While otherwise an attractive measure of metacognition, meta-*d'* requires that the primary task be conducted in a manner analogous to a signal detection task (e.g. 2AFC). Thus, it is not suitable as a measure of metacognition in free-response tasks, or, in its current implementation, multiple-choice tasks where the number of choice alternatives exceed 2. Nevertheless, the general implication of that approach is the same as elucidated by our results: In certain tasks, metrics of resolution are inherently related to performance on the primary task.
...
rocs <- group_by(trivia, condition, session) %>% 
  ...
  group_modify(~data.frame(sdt_roc(.$accuracy, .$rating, min=1,max=10, n_bins = 10, constant = 0)))
zrocs <- group_by(trivia, condition, session) %>% 
  mutate(rating = rating*10) %>% 
  group_modify(~data.frame(qnorm(sdt_roc(.$accuracy, .$rating, min=1,max=10, n_bins = 10, constant = 0))))
