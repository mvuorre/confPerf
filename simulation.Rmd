...
However, in mAFC tasks, gamma is downward biased as a function of m (number of response alternatives) [@schwartz1994methodological]. Below, we show in detail how guesses cause this bias.
...
This simulation produces hypothetical data sets of responses (accuracy & confidence rating pairs) under various conditions of guessing, performance, and true resolution. The simulation scheme is adapted from @benjamin2008measurement, and is outlined for one hypothetical subject as follows:

- A list of items is generated based on number of items, and proportion of known items (Knowledge = *K*)
- For each item, accuracy is determined such that known items are always correct, whereas unknown items are correct according to a defined guessing probability *G*
- For each item, a "metacognitive evidence" value is generated from one of two gaussian distributions depending on whether the item was known or unknown
  ...
  - Known item evidence values: mean *R* and standard deviation $\sigma_k$. Thus, R defines the generative resolution parameter as the difference between the evidence distributions.
- Evidence values are converted to confidence ratings (6-category scale) values via five thresholds
- Resolution metrics are obtained based on this simulated data set
...
tibble(
  Variable = c(
    ...
    "Proportion of known items (knowledge)",
...
```{r sim-test, fig.cap="Example run of simulation scheme with $R = 1$: Histogram of simulated confidence ratings split by accuracy."}
tmp <- sim_run(
  out = "data", 
  d_gen = 1,
  p_know = .65,
...
dat_metrics %>% 
  ...
  labs(x="Observed proportion correct", y="Corrected p(correct) ('knowledge')") +
...
```{r regression-test, layout="l-page", fig.width=8, fig.height=3.6, fig.cap="Comparison of observed correlations to simulation results."}
DEGREE <- 2
# PLot
tmp <- dat_metrics %>% 
  group_by(exp) %>% 
...
tmp %>% 
  ...
  scale_color_viridis_c("Resolution", 
                        values = rescale(tmp$d_gen, 
                                         to = c(0, 1))) +
  theme(panel.spacing = unit(11, "pt"))
...
tmp <- dat_metrics %>% 
  group_by(exp) %>% 
  summarise(
    p_know = mean(p, na.rm = T),
    gamma = mean(gamma, na.rm = T), 
    da = mean(da, na.rm = T)
    ) %>% 
  group_by(exp) %>% 
  # Get the nearest performance used in simulations
  mutate(p_know = p_know_bins[which.min(abs(p_know - p_know_bins))]) %>% 
  ungroup() %>% 
  gather(parameter, mean_data_estimate, gamma, da)
tmp2 <- sims_long_bias %>% 
  filter(between(p_know, .65, .76))
# Take rows for each exp and parameter where distance between observed resolution value and estimated(guessing=0.25) is the smalles
full_join(tmp, tmp2) %>% 
  mutate(difference = abs(mean_data_estimate - p_guess0.25)) %>% 
  arrange(parameter, exp, difference) %>%
  distinct(parameter, exp, .keep_all = TRUE) %>% 
  select(exp, p_know, parameter, mean_data_estimate, p_guess0.25, p_guess0) %>% 
  mutate(percent_increase = (p_guess0-mean_data_estimate)/mean_data_estimate) %>% 
  group_by(parameter) %>% 
  mutate(mpc = mean(percent_increase)) %>% 
  kable(
    escape = FALSE, 
    digits = 2,
    caption = "Resolution metrics from data"
  )
```

### Test subjects against simulated curve at perfect metacognition

Here, we attempt to test observed gamma/da against perfect metacognition. Each participant's observed gamma/da is paired with the corresponding mean simulated estimate at maximum d_gen (corresponding to about gamma = .96) and guessing = 0.25 and the participant's level of performance.

```{r}
tmp1 <- dat_metrics %>% 
  select(exp, subject, gamma, da, p_know) %>% 
  group_by(exp, subject) %>% 
  mutate(p_know_bin = p_know_bins[which.min(abs(p_know - p_know_bins))]) %>% 
  ungroup()
tmp2 <- sims_mean %>% 
  filter(d_gen == max(delta_bins), p_guess == "0.25")
tmp <- left_join(tmp1, tmp2, by = c("p_know_bin" = "p_know"))
names(tmp) <- str_replace_all(names(tmp), "\\.x", ".data")
names(tmp) <- str_replace_all(names(tmp), "\\.y", ".perfect")
...
t.test(tmp$gamma.perfect, tmp$gamma.data, paired = T)
t.test(tmp$da.perfect, tmp$da.data, paired = T)
p1 <- tmp %>% 
  mutate(
    gamma_diff = gamma.perfect - gamma.data,
    da_diff = da.perfect - da.data
  ) %>% 
  select(exp, subject, contains("_diff")) %>% 
  gather(key, value, gamma_diff, da_diff) %>% 
  ggplot(aes(value)) +
  geom_histogram(bins = 50) +
  facet_wrap("key", scales = "free")
p2 <- tmp %>% 
  ggplot(aes(gamma.perfect, gamma.data)) +
  geom_abline(lty = 2) +
  geom_point()
p3 <- tmp %>% 
  ggplot(aes(da.perfect, da.data)) +
  geom_abline(lty = 2) +
  geom_point()
(p3 | p2) / p1
```
...
sims_bd %>% 
  ...
  scale_x_continuous(Resolution, breaks = pretty_breaks(5)) +
  scale_y_continuous("Mean estimate", breaks = pretty_breaks(5)) +
