...
This simulation examines potential bias in measures of metacognitive resolution due to guessing and task knowledge.
...
We then tested whether the performance-resolution metric regression slopes were significantly steeper in the data, vs the equivalent slope in the simulations.
...
```{r regression-test-fig, layout="l-page", fig.width=8, fig.height=3.6, fig.cap="Comparison of observed correlations to simulation results."}
DEGREE <- 1
# PLot
tmp <- dat_metrics %>% 
  group_by(exp) %>% 
...
```{r regression-test}
# Simulated slope at each experiment's mean d_ord
tmp1 <- dat_metrics %>% 
  group_by(exp) %>% 
  ...
  nest(-exp, -parameter) %>% 
  mutate(
    res = map(data, ~lm(p_guess0.25 ~ p_know, data = .)),
    out = map(res, ~tidy(.))
  ) %>% 
  unnest(out) %>% 
  select(exp, parameter, term, sim_coef = estimate)

# Observed slope for each experiment
tmp2 <- dat_metrics %>% 
  mutate(gamma = gz) %>% 
  gather(parameter, value, gamma:da) %>% 
  nest(-exp, -parameter) %>% 
  mutate(
    res = map(data, ~lm(value ~ p_know, data = .)),
    out = map(res, ~tidy(.)),
...
left_join(tmp1, tmp2) %>% 
  mutate(
    tval = (data_coef - sim_coef) / std.error,
    # One-sided p-values
    pval = pt(tval, df = N_subj-2, lower=FALSE)
  ) %>% 
...
```{r}
tmp1 <- dat_metrics %>% 
  select(exp, subject, gamma = gz, da, p_know) %>% 
...
```{r}
# Difference from perfect on performance
# (How much below perfect are they as function of performance)
p4 <- tmp %>% 
  mutate(
    gamma_diff = gamma.perfect - gamma.data,
    da_diff = da.perfect - da.data
  ) %>% 
  select(exp, subject, p_know, contains("_diff")) %>% 
  gather(key, value, gamma_diff, da_diff)
p4 %>% 
  ggplot(aes(p_know, value)) +
  geom_point() +
  geom_smooth() +
...
## Appendix A: Replication of Benjamin and Diaz
...
## Appendix B: Simulation function {.appendix}

```{r show-simulation-function, ref.label="simulation-function", echo=TRUE}
```

## Appendix C: Calculating gamma from ROC {.appendix}
...
(tmpp2 + geom_point(aes(y=gamma_zroc)) | 
  ...
  coord_cartesian(ylim=-1:1, xlim = -1:1)
